...
- For every new helper in the diff, grep `pkg/runtime` and `pkg/jsc` for the same
  *concept* under a different name (`bytes_all_ascii` vs an inline loop; a second
...
Ask of each structural change: *is there a version of this with zero new concepts?*
...
- Comment quality: this codebase explains *why* at every non-obvious decision.
