...
maintenance, and usually *slower* than a tuned SDK routine. Your default answer is
...
1. **In-repo** — `pkg/runtime/*.odin`, `pkg/jsc`, `pkg/runtime/eventloop`,
   ...
   frequent defect here. Grep for the *concept*, not the name the diff chose
...
- **Semantics vs Node.** Does it produce what Node produces (error taxonomy,
  ...
  empty-input behavior is a *wrap* candidate, not a drop-in.
