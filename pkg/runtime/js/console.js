(function (out, err) {
	...
	function inspect(v, seen, depth) {
		...
		if (typeof Set !== "undefined" && v instanceof Set) {
			var st = [];
			v.forEach(function (val) { st.push(inspect(val, seen, depth + 1)); });
		...
		if (typeof first === "string" && first.indexOf("%") !== -1) {
			...
			for (var p = 0; p < first.length; p++) {
				...
				if (ch === "%" && p + 1 < first.length) {
					...
					if (spec === "%") { result += "%"; p++; continue; }
					if ("sdifjoOc".indexOf(spec) !== -1) {
						if (i >= args.length && spec !== "%") { result += "%" + spec; p++; continue; }
						var a = args[i++];
						p++;
						if (spec === "s") result += typeof a === "string" ? a : stringify(a);
						else if (spec === "d" || spec === "i") result += typeof a === "bigint" ? String(a) + "n" : (typeof a === "symbol" ? "NaN" : String(Math.trunc(Number(a))));
						else if (spec === "f") result += typeof a === "symbol" ? "NaN" : String(parseFloat(a));
						else if (spec === "j") { try { result += JSON.stringify(a); } catch (e) { result += "[Circular]"; } }
	...
	function line(writer, args) { emit(writer, formatArgs(args)); }
	...
	var kSecond = 1000, kMinute = 60 * kSecond, kHour = 60 * kMinute;
	function pad(value) { return String(value).padStart(2, "0"); }
	function duration(ms) {
		var hours = 0, minutes = 0, seconds = 0;
		if (ms >= kSecond) {
			if (ms >= kMinute) {
				if (ms >= kHour) { hours = Math.floor(ms / kHour); ms = ms % kHour; }
				minutes = Math.floor(ms / kMinute); ms = ms % kMinute;
			}
			seconds = ms / kSecond;
		}
		if (hours !== 0 || minutes !== 0) {
			...
			return res + ":" + pad(parts[0]) + "." + parts[1] + " (" + (hours !== 0 ? "h:m" : "") + "m:ss.mmm)";
		}
	...
	function table(data, columnsFilter) {
		if (data === null || typeof data !== "object") { line(out, [data]); return; }
		...
		for (var r = 0; r < indexKeys.length; r++) {
			...
			if (item !== null && typeof item === "object" && !(item instanceof Date) && !(item instanceof RegExp)) {
		...
		function rowText(cells) {
			var parts = [];
			for (var c3 = 0; c3 < widths.length; c3++) parts.push(pad(cells[c3] === undefined ? "" : cells[c3], widths[c3]));
			return "│" + parts.join("│") + "│";
		}
	...
	function label(x) { return x === undefined ? "default" : String(x); }

	var console = {
		log: function () { line(out, args(arguments)); },
		info: function () { line(out, args(arguments)); },
		debug: function () { line(out, args(arguments)); },
		dirxml: function () { line(out, args(arguments)); },
		error: function () { line(err, args(arguments)); },
		warn: function () { line(err, args(arguments)); },
		dir: function (obj) { emit(out, inspect(obj, [], 0)); },
		trace: function () {
		...
		countReset: function (name) {
			var key = label(name);
			if (counts[key] === undefined) { emit(err, "Warning: Count for '" + key + "' does not exist"); return; }
			delete counts[key];
		},
		group: function () { if (arguments.length) line(out, args(arguments)); groupIndent += "  "; },
		groupCollapsed: function () { if (arguments.length) line(out, args(arguments)); groupIndent += "  "; },
		groupEnd: function () { groupIndent = groupIndent.slice(0, -2); },
		time: function (name) {
			var key = label(name);
			if (timers[key] !== undefined) { emit(err, "Warning: Label '" + key + "' already exists for console.time()"); return; }
			timers[key] = Date.now();
		},
		timeEnd: function (name) {
			var key = label(name);
			if (timers[key] === undefined) { emit(err, "Warning: No such label '" + key + "' for console.timeEnd()"); return; }
		...
		timeLog: function (name) {
			var key = label(name);
			if (timers[key] === undefined) { emit(err, "Warning: No such label '" + key + "' for console.timeLog()"); return; }
			var d = Date.now() - timers[key];
			var extra = Array.prototype.slice.call(arguments, 1);
			emit(out, key + ": " + duration(d) + (extra.length ? " " + formatArgs(extra) : ""));
		},
		clear: function () { out("[2J[3J[H"); },
		table: function (data, columns) { table(data, columns); }
	};
