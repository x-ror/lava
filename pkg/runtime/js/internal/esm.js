	...
	function transform(source, url, filename, dirname) {
		...
		function transformImport(stmt) {
			...
			if (dm) {
				...
				return "var " + t1 + " = " + req + "; var " + dm[1] + " = __importDefault(" + t1 +
					"); var " + dm[2] + " = __importStar(" + t1 + ");";
			}
			// default, { named }
			dm = clause.match(new RegExp("^(" + IDENT + ")\\s*,\\s*\\{([\\s\\S]*)\\}$"));
			if (dm) {
				var t2 = nextTemp("__lava_imp_");
				return "var " + t2 + " = " + req + "; var " + dm[1] + " = __importDefault(" + t2 +
					"); var { " + destructurePattern(dm[2]) + " } = " + t2 + ";";
			}
			// * as ns
		...
		function transformExport(stmt) {
			...
			if (m) {
				...
				for (var i = 0; i < pairs.length; i++) {
					s += " module.exports[" + jsonString(pairs[i].exported) + "] = " + t +
			...
			if (m) {
				...
				return {
					body: "{ var " + ta + " = require(" + jsonString(m[2]) +
						"); for (var __k in " + ta + ") { if (__k !== \"default\") module.exports[__k] = " +
						ta + "[__k]; } }",
				};
			}
			...
			if (m) {
				...
				for (var j = 0; j < locals.length; j++) {
					t2.push("module.exports[" + jsonString(locals[j].exported) + "] = " + locals[j].local + ";");
				}
				return { body: "", tail: t2 };
			}
			...
			m = stmt.match(new RegExp("^export\\s+((?:async\\s+function|function|class)\\s+(" + IDENT + ")[\\s\\S]*)$"));
			if (m) {
				return { body: m[1], tail: ["module.exports[" + jsonString(m[2]) + "] = " + m[2] + ";"] };
			}
			return null;
