...
(function (require, module, exports, native) {
	...
	function bytesToString(bytes, encoding) {
		...
		if (encoding === "ascii" || encoding === "latin1") {
			var s = "";
			for (var i = 0; i < bytes.length; i++) s += String.fromCharCode(encoding === "ascii" ? bytes[i] & 0x7f : bytes[i]);
			return s;
		}
		throw new TypeError("Unknown encoding: " + encoding);
	}
	...
	function checkBounds(buf, offset, byteLength) {
		...
		if (offset < 0 || offset + byteLength > buf.length) throw new RangeError("Offset is outside the bounds of the Buffer");
		return offset;
	}
	...
	function bidirectionalIndexOf(buf, value, byteOffset, encoding, forward) {
		...
		if (needle.length === 0) return forward ? clampIndex(byteOffset, buf.length, 0) : Math.min(clampIndex(byteOffset, buf.length, buf.length), buf.length);
		var start = clampIndex(byteOffset, buf.length, forward ? 0 : buf.length);
		if (forward) {
			for (var i = start; i <= buf.length - needle.length; i++) {
				var ok = true;
				for (var j = 0; j < needle.length; j++) if (buf[i + j] !== needle[j]) { ok = false; break; }
				if (ok) return i;
			}
		} else {
			for (var k = Math.min(start, buf.length - needle.length); k >= 0; k--) {
				var match = true;
				for (var m = 0; m < needle.length; m++) if (buf[k + m] !== needle[m]) { match = false; break; }
				if (match) return k;
			}
		}
		return -1;
	...
	class Buffer extends Uint8Array {
		...
		write(string, offset, length, encoding) {
			...
			length = length === undefined ? bytes.length : Math.min(toInteger(length, bytes.length), bytes.length);
		...
		compare(target, targetStart, targetEnd, sourceStart, sourceEnd) {
			if (!(target instanceof Uint8Array)) throw new TypeError("target must be a Buffer or Uint8Array");
			targetStart = validateRange(targetStart, K_MAX_LENGTH, "targetStart");
			targetEnd = validateRange(targetEnd === undefined ? target.length : targetEnd, target.length, "targetEnd");
			sourceStart = validateRange(sourceStart, K_MAX_LENGTH, "sourceStart");
			sourceEnd = validateRange(sourceEnd === undefined ? this.length : sourceEnd, this.length, "sourceEnd");
		...
		swap16() {
			...
			for (var i = 0; i < this.length; i += 2) {
				var a = this[i]; this[i] = this[i + 1]; this[i + 1] = a;
			}
			return this;
		}
		...
		swap64() {
			...
			for (var i = 0; i < this.length; i += 8) {
				for (var j = 0; j < 4; j++) {
					var a = this[i + j]; this[i + j] = this[i + 7 - j]; this[i + 7 - j] = a;
				}
			}
			return this;
		}
	...
	p.inspect = function () { return "<Buffer " + this.toString("hex").replace(/(..)/g, "$1 ").trim() + ">"; };
	p.toLocaleString = p.toString;
	p.utf8Slice = function (start, end) { return this.toString("utf8", start, end); };
	p.hexSlice = function (start, end) { return this.toString("hex", start, end); };
	p.asciiSlice = function (start, end) { return this.toString("ascii", start, end); };
	p.latin1Slice = function (start, end) { return this.toString("latin1", start, end); };
	p.base64Slice = function (start, end) { return this.toString("base64", start, end); };
	p.base64urlSlice = function (start, end) { return this.toString("base64url", start, end); };
	p.ucs2Slice = function (start, end) { return this.toString("utf16le", start, end); };
	p.utf8Write = function (string, offset, length) { return this.write(string, offset, length, "utf8"); };
	p.hexWrite = function (string, offset, length) { return this.write(string, offset, length, "hex"); };
	p.asciiWrite = function (string, offset, length) { return this.write(string, offset, length, "ascii"); };
	p.latin1Write = function (string, offset, length) { return this.write(string, offset, length, "latin1"); };
	p.base64Write = function (string, offset, length) { return this.write(string, offset, length, "base64"); };
	p.base64urlWrite = function (string, offset, length) { return this.write(string, offset, length, "base64url"); };
	p.ucs2Write = function (string, offset, length) { return this.write(string, offset, length, "utf16le"); };
	p.readUInt8 = p.readUint8 = function (offset) { offset = checkBounds(this, offset, 1); return this[offset]; };
	p.readInt8 = function (offset) { var v = this.readUInt8(offset); return v & 0x80 ? v - 0x100 : v; };
	p.readUIntLE = p.readUintLE = function (offset, byteLength) { return readUInt(this, offset, byteLength, true); };
	p.readUIntBE = p.readUintBE = function (offset, byteLength) { return readUInt(this, offset, byteLength, false); };
	p.readIntLE = function (offset, byteLength) { return readInt(this, offset, byteLength, true); };
	p.readIntBE = function (offset, byteLength) { return readInt(this, offset, byteLength, false); };
	p.readUInt16LE = p.readUint16LE = function (offset) { return viewOf(this).getUint16(checkBounds(this, offset, 2), true); };
	p.readUInt16BE = p.readUint16BE = function (offset) { return viewOf(this).getUint16(checkBounds(this, offset, 2), false); };
	p.readUInt32LE = p.readUint32LE = function (offset) { return viewOf(this).getUint32(checkBounds(this, offset, 4), true); };
	p.readUInt32BE = p.readUint32BE = function (offset) { return viewOf(this).getUint32(checkBounds(this, offset, 4), false); };
	p.readInt16LE = function (offset) { return viewOf(this).getInt16(checkBounds(this, offset, 2), true); };
	p.readInt16BE = function (offset) { return viewOf(this).getInt16(checkBounds(this, offset, 2), false); };
	p.readInt32LE = function (offset) { return viewOf(this).getInt32(checkBounds(this, offset, 4), true); };
	p.readInt32BE = function (offset) { return viewOf(this).getInt32(checkBounds(this, offset, 4), false); };
	p.readFloatLE = function (offset) { return viewOf(this).getFloat32(checkBounds(this, offset, 4), true); };
	p.readFloatBE = function (offset) { return viewOf(this).getFloat32(checkBounds(this, offset, 4), false); };
	p.readDoubleLE = function (offset) { return viewOf(this).getFloat64(checkBounds(this, offset, 8), true); };
	p.readDoubleBE = function (offset) { return viewOf(this).getFloat64(checkBounds(this, offset, 8), false); };
	if (typeof BigInt === "function") {
		p.readBigUInt64LE = p.readBigUint64LE = function (offset) { return readBigUInt(this, offset, true); };
		p.readBigUInt64BE = p.readBigUint64BE = function (offset) { return readBigUInt(this, offset, false); };
		p.readBigInt64LE = function (offset) { return readBigInt(this, offset, true); };
		p.readBigInt64BE = function (offset) { return readBigInt(this, offset, false); };
	}

	p.writeUInt8 = p.writeUint8 = function (value, offset) { offset = checkBounds(this, offset, 1); this[offset] = Number(value) & 0xff; return offset + 1; };
	p.writeInt8 = function (value, offset) { return this.writeUInt8(value, offset); };
	p.writeUIntLE = p.writeUintLE = function (value, offset, byteLength) { return writeUInt(this, value, offset, byteLength, true); };
	p.writeUIntBE = p.writeUintBE = function (value, offset, byteLength) { return writeUInt(this, value, offset, byteLength, false); };
	p.writeIntLE = function (value, offset, byteLength) { return writeInt(this, value, offset, byteLength, true); };
	p.writeIntBE = function (value, offset, byteLength) { return writeInt(this, value, offset, byteLength, false); };
	p.writeUInt16LE = p.writeUint16LE = function (value, offset) { offset = checkBounds(this, offset, 2); viewOf(this).setUint16(offset, Number(value), true); return offset + 2; };
	p.writeUInt16BE = p.writeUint16BE = function (value, offset) { offset = checkBounds(this, offset, 2); viewOf(this).setUint16(offset, Number(value), false); return offset + 2; };
	p.writeUInt32LE = p.writeUint32LE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setUint32(offset, Number(value), true); return offset + 4; };
	p.writeUInt32BE = p.writeUint32BE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setUint32(offset, Number(value), false); return offset + 4; };
	p.writeInt16LE = function (value, offset) { offset = checkBounds(this, offset, 2); viewOf(this).setInt16(offset, Number(value), true); return offset + 2; };
	p.writeInt16BE = function (value, offset) { offset = checkBounds(this, offset, 2); viewOf(this).setInt16(offset, Number(value), false); return offset + 2; };
	p.writeInt32LE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setInt32(offset, Number(value), true); return offset + 4; };
	p.writeInt32BE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setInt32(offset, Number(value), false); return offset + 4; };
	p.writeFloatLE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setFloat32(offset, Number(value), true); return offset + 4; };
	p.writeFloatBE = function (value, offset) { offset = checkBounds(this, offset, 4); viewOf(this).setFloat32(offset, Number(value), false); return offset + 4; };
	p.writeDoubleLE = function (value, offset) { offset = checkBounds(this, offset, 8); viewOf(this).setFloat64(offset, Number(value), true); return offset + 8; };
	p.writeDoubleBE = function (value, offset) { offset = checkBounds(this, offset, 8); viewOf(this).setFloat64(offset, Number(value), false); return offset + 8; };
	if (typeof BigInt === "function") {
		p.writeBigUInt64LE = p.writeBigUint64LE = function (value, offset) { return writeBigUInt(this, value, offset, true); };
		p.writeBigUInt64BE = p.writeBigUint64BE = function (value, offset) { return writeBigUInt(this, value, offset, false); };
		p.writeBigInt64LE = function (value, offset) { return writeBigInt(this, value, offset, true); };
		p.writeBigInt64BE = function (value, offset) { return writeBigInt(this, value, offset, false); };
	}

	Object.defineProperty(p, "parent", { get: function () { return this.buffer; } });
	Object.defineProperty(p, "offset", { get: function () { return this.byteOffset; } });
	...
	Buffer.compare = function (a, b) {
		if (!(a instanceof Uint8Array) || !(b instanceof Uint8Array)) throw new TypeError("Arguments must be Buffers or Uint8Arrays");
	...
	function isAscii(input) {
		...
		if (!(bytes instanceof Uint8Array)) throw new TypeError("input must be a Buffer, ArrayBuffer, or TypedArray");
	...
	Object.defineProperty(exported, "INSPECT_MAX_BYTES", {
		get: function () { return inspectMaxBytes; },
		set: function (value) { inspectMaxBytes = value; },
	});
