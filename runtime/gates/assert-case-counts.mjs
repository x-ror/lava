#!/usr/bin/env node
// Agent system — case-count floors — assert oracle/bench file counts against case-counts.json.
// Sourced by runners via: node runtime/gates/assert-case-counts.mjs <suite-key>
// Or: node runtime/gates/assert-case-counts.mjs --all
...
function assertSuite(key, spec) {
  ...
  if (n < spec.min) {
    console.error(
      `gate-integrity case-count: ${key} has ${n} cases, expected >= ${spec.min} (see runtime/gates/case-counts.json)`,
    );
    process.exit(1);
  }
