...
- One intentional divergence from Node: because JavaScript `ArrayBuffer`s are always zero-initialized, `allocUnsafe`/`allocUnsafeSlow` allocations *at or above* the pool threshold (their own backing store) come back zero-filled rather than holding arbitrary memory. Only the pooled (sub-threshold) path exposes uninitialized bytes the way Node's `allocUnsafe` always does. This is strictly safer and observable only by code that reads uninitialized memory, which is already undefined behavior.
