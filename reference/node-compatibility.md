...
| **buffer** | ✅ | ✅ | `Buffer` (full read/write/encode/search, Node-coded errors, `util.inspect` `<Buffer ..>`, `poolSize` allocation pooling), `Blob`, `File`, `SlowBuffer`, `atob`/`btoa` (module-scoped), `isAscii`/`isUtf8`, `transcode`, `resolveObjectURL` (+ global `URL.createObjectURL`/`revokeObjectURL`) | large `allocUnsafe`/`allocUnsafeSlow`/`SlowBuffer` are natively uninitialized; oversized `Buffer`/typed-array/`ArrayBuffer` allocations throw a catchable `RangeError` at an enforced ceiling (`LAVA_MAX_BUFFER_BYTES`-tunable) instead of aborting JSC — see notes | [buffer.odin](../pkg/runtime/buffer.odin), [js/internal/buffer.js](../pkg/runtime/js/internal/buffer.js) |
...
- **Oversized-allocation ceiling (`kMaxLength`).** Node/V8 advertise `kMaxLength` as `Number.MAX_SAFE_INTEGER` on 64-bit and only *attempt* an allocation that large, throwing a catchable `RangeError` if it cannot be satisfied. On JavaScriptCore an in-range request it cannot allocate can instead **abort the process**, so there is nothing to catch. Lava therefore treats `kMaxLength` as a hard, enforced ceiling — set to the practical JSC array-buffer limit (**4 GiB on 64-bit, 2 GiB − 1 on 32-bit**). This matches **Bun**, the other JSC-based runtime, which likewise reports `kMaxLength === 4294967296` rather than `MAX_SAFE_INTEGER`; it is the JSC-family convention, not a lava-only choice. `Buffer.alloc`/`allocUnsafe`/`allocUnsafeSlow`/`concat`, the `Buffer` constructor, *and* the global `ArrayBuffer`/`SharedArrayBuffer`/`TypedArray` constructors all throw a catchable `RangeError` for any request beyond it **before** JSC is asked to allocate. Buffer paths keep Node's `ERR_OUT_OF_RANGE` code (same as Bun); raw typed-array/`ArrayBuffer` requests throw an uncoded `RangeError: Out of memory` — the same shape Bun's JSC produces (a length above `2**53 − 1` keeps JSC's distinct `Invalid …` message, since that is rejected by the native constructor). The resizable-buffer `maxByteLength` option is validated the same way (Bun rejects an oversized reservation; Node does not). The ceiling is overridable with the `LAVA_MAX_BUFFER_BYTES` environment variable (a byte count; used by the alloc-guard smoke test to exercise the cap without allocating gigabytes, and by operators who want a tighter bound; a non-positive value is ignored). Negative/`NaN`/non-number sizes are still validated and throw before any allocation. Residual difference: a request *within* the ceiling that still exhausts available RAM can abort under JSC, where V8 would throw — set `LAVA_MAX_BUFFER_BYTES` lower if your workload needs a guaranteed-catchable bound.
