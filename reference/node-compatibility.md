...
- **Oversized-allocation ceiling (`kMaxLength`).** Node advertises `kMaxLength` as `Number.MAX_SAFE_INTEGER` on 64-bit and only *attempts* an allocation that large, throwing a catchable `RangeError` if it cannot be satisfied. JavaScriptCore instead **aborts the process** on an in-range request it cannot allocate, so there is nothing to catch. Lava therefore treats `kMaxLength` as a hard, enforced ceiling: it is set to the practical JSC array-buffer limit (**4 GiB on 64-bit, 2 GiB − 1 on 32-bit**), and `Buffer.alloc`/`allocUnsafe`/`allocUnsafeSlow`/`concat`, the `Buffer` constructor, *and* the global `ArrayBuffer`/`SharedArrayBuffer`/`TypedArray` constructors all throw a catchable `RangeError` for any request beyond it **before** JSC is asked to allocate (Buffer paths keep Node's `ERR_OUT_OF_RANGE` code; raw typed-array/`ArrayBuffer` requests throw a plain `RangeError`, matching V8's uncoded "Invalid typed array length"). The ceiling is overridable with the `LAVA_MAX_BUFFER_BYTES` environment variable (a byte count; used by tests to exercise the cap without allocating gigabytes, and by operators who want a tighter bound). Negative/`NaN`/non-number sizes are still validated and throw before any allocation. Residual difference: a request *within* the ceiling that still exhausts available RAM can abort under JSC, where V8 would throw — set `LAVA_MAX_BUFFER_BYTES` lower if your workload needs a guaranteed-catchable bound.
